...
def decimalDate(date,fmt="%Y-%m-%d",variable=False):
    ...
    if fmt == "":
        return date
    delimiter=re.search(r'[^0-9A-Za-z%]',fmt) ## search for non-alphanumeric symbols in fmt (should be field delimiter)
    delimit=None
...
class tree: ## tree class
    ...
    def plotPoints(self,ax,x_attr=None,y_attr=None,target=None,size=None,colour=None,
               zorder=None,outline=None,outline_size=None,outline_colour=None,edgecolor="none",**kwargs):
        """
        ...
        **kwargs: Additional keyword arguments to pass to the `ax.scatter` method.
        ...
        ax.scatter(xs,ys,s=sizes,facecolor=colours,edgecolor=edgecolor,zorder=zorder,**kwargs) ## put a circle at each tip
        if outline:
...
def loadNewick(tree_path,tip_regex=r'\|([0-9]+\-[0-9]+\-[0-9]+)',date_fmt='%Y-%m-%d',variableDate=True,absoluteTime=False,verbose=False, sortBranches = True):
    """
...
def loadNexus(tree_path,tip_regex=r'\|([0-9]+\-[0-9]+\-[0-9]+)',date_fmt='%Y-%m-%d',treestring_regex=r'tree [A-Za-z\_]+([0-9]+)',variableDate=True,absoluteTime=True,verbose=False, sortBranches=True):
    """
    ...
    for line in handle:
        ...
        match=re.search(r'Dimensions ntax=([0-9]+);',l)
        if match:
...
def loadJSON(json_object,json_translation={'name':'name','absoluteTime':'num_date'},verbose=False,sort=True,stats=True):
    ...
    assert 'name' in json_translation and any(key in json_translation for key in length_keys),'JSON translation dictionary missing entries: %s'%(', '.join([entry for entry in ['name']+length_keys if (entry in json_translation)==False]))
